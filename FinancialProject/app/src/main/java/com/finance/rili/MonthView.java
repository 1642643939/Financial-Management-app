...
public abstract class MonthView extends View {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        width = widthSize;
        NUM_ROWS = 6; //本来是想根据每月的行数，动�?�改变控件高度，现在为了使滑动的左右两边效果相同，不适用getMonthRowNumber();
    ...
    public void smoothScrollBy(int dx, int dy) {
        ...
        invalidate();//这里必须调用invalidate()才能保证computeScroll()会被调用，否则不�?定会刷新界面，看不到滚动效果
    }
    ...
    private void setLeftDate(){
        ...
        int month = selMonth;
        int day = selDay;
        if(month == 0){//若果�?1月份，则变成12月份
            year = selYear-1;
            month = 11;
        }else if(DateUtils.getMonthDays(year, month-1) < day){//向左滑动，当前月天数小于左边�?
            //如果当前日期为该月最后一点，当向前推的时候，就需要改变�?�中的日�?
            month = month-1;
    ...
    private void setRightDate(){
        ...
        int month = selMonth;
        int day = selDay;
        if(month == 11){//若果�?12月份，则变成1月份
            year = selYear + 1;
            month = 0;
        }else if(DateUtils.getMonthDays(year, month + 1) < day){//向右滑动，当前月天数小于左边�?
            //如果当前日期为该月最后一点，当向前推的时候，就需要改变�?�中的日�?
            month = month + 1;
            day = DateUtils.getMonthDays(year, month);
        }else{
