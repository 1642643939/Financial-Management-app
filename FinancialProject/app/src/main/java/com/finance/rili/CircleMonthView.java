...
//        }
    ...
    protected void drawText(Canvas canvas, int column, int row, int year,int month,int day) {
        ...
        if(day== selDay){//日期为�?�中的日�?
            if(!TextUtils.isEmpty(des)){//desc不为空的时�??
        ...
        }else if(day== currDay && currDay != selDay && currMonth == selMonth){//今日的颜色，不是选中的时�?
            //正常月，选中其他日期，则今日为红�?
            paint.setColor(theme.colorToday());
            canvas.drawText(day+"", startX, startY, paint);
        }else{
            if(!TextUtils.isEmpty(des)){//没�?�中，但是desc不为�?
                int dateY = (int) (startY - 10);
